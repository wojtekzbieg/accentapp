...
struct OverallFeedbackView: View {
    ...
    var body: some View {
        VStack {
            ...
            HStack(spacing: 16) {
                Button(action: {
                    // Play full sentence with Coach voice (normal speed)
                    let fullSentence = scores.words?.compactMap { $0.word }.joined(separator: " ") ?? ""
                    
                    viewModel.playTextToSpeech(text: fullSentence)
                }) {
                    HStack(spacing: 8) {
                ...
                Button(action: {
                    // Play full sentence with slow speed
                    let fullSentence = scores.words?.compactMap { $0.word }.joined(separator: " ") ?? ""
                    
                    viewModel.playTextToSpeech(text: fullSentence, rate: 0.01)
                }) {
